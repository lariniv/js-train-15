...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < 8; i++) {
    let sym = characters[Math.floor(Math.random() * characters.length)];
    password += sym;
  }
  return password;
...
function getFractionalPart(num) {
  if (typeof num === "number") {
    ...
    let fractionalNum = num - roundedNum;
    return Math.fround(fractionalNum);
  } else {
...
function compareAndRound(num1, num2) {
  if (typeof num1 === "number" && typeof num2 === "number") {
    let maxNum = Math.round(Math.max(num1, num2));
    return maxNum;
  } else {
